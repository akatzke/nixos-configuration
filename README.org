...
:PROPERTIES:
:header-args: :results none :tangle system-configuration/core.nix
:END:
...
** Setup
Set up the system-configuration files that shall be generated:
#+begin_src nix :noweb yes :tangle system-configuration/core.nix
<<system-config-preamble>>
#+end_src

#+begin_src nix :noweb yes :tangle system-configuration/desktop.nix
<<system-config-preamble>>
#+end_src

#+begin_src nix :noweb yes :tangle system-configuration/hephaestus.nix
<<system-config-preamble>>
#+end_src

#+begin_src nix :noweb yes :tangle system-configuration/orpheus.nix
<<system-config-preamble>>
#+end_src

#+begin_src nix :noweb yes :tangle system-configuration/sisyphus.nix
<<system-config-preamble>>
#+end_src

#+begin_src nix :noweb yes :tangle system-configuration/non-iso.nix
<<system-config-preamble>>
#+end_src

#+begin_src nix :noweb yes :tangle system-configuration/uefi-boot.nix
<<system-config-preamble>>
#+end_src
...
#+begin_src nix :tangle system-configuration/hephaestus.nix
networking.hostName = "hephaestus"; # Define your hostname.
#+end_src
#+begin_src nix :tangle system-configuration/orpheus.nix
networking.hostName = "orpheus"; # Define your hostname.
#+end_src
#+begin_src nix :tangle system-configuration/sisyphus.nix
...
#+begin_src nix :tangle system-configuration/uefi-boot.nix
boot.loader = {
  timeout = 2;
...
#+begin_src nix :tangle system-configuration/desktop.nix
...
#+begin_src nix :tangle system-configuration/non-iso.nix
networking = {
...
**** Hephaestus
:PROPERTIES:
:header-args: :tangle system-configuration/hephaestus.nix
:END:
...
**** Sisyphus
:PROPERTIES:
:header-args: :tangle system-configuration/sisyphus.nix
:END:
Mount external HDD
#+begin_src nix
fileSystems."/mnt/HDD" = {
...
Bind the =Media= folder onto =/srv/nfs/Media= for ~NFS~ file sharing.
See [[NFS]]
#+begin_src nix
...
*** Devices
#+begin_src nix :tangle syncthing/devices.nix
{
  "hephaestus" = {
...
*** Folder Weaving
#+begin_src nix :noweb yes :tangle syncthing/folders-desktop.nix
let
  root = "/home/yusu";
  home = "/home/yusu";
in  {
...
#+begin_src nix :noweb yes :tangle syncthing/folders-sisyphus.nix
let
...
Link the secret (and encrypted) =authinfo= file, which stores logins that [[id:6288c98c-cef6-42a1-ac0c-96952ad5a385][Doom-Emacs]] shall remember.
#+begin_src nix :tangle system-configuration/desktop.nix
...
#+end_src
** Finalize
#+begin_src nix :tangle system-configuration/core.nix
}
#+end_src
...
* Home Manager
:PROPERTIES:
:header-args: :tangle home-manager/core.nix
:END:
...
#+begin_src nix :tangle home-manager/core.nix
...
#+begin_src nix :tangle home-manager/desktop.nix
{ osConfig, config, pkgs, ... }:
let
  user = "yusu";
  name = "Jonas Opitz";
...
#+begin_src nix :tangle home-manager/hephaestus.nix
{ osConfig, config, pkgs, ... }:
let
  user = "yusu";
  name = "Jonas Opitz";
...
#+begin_src nix :tangle home-manager/orpheus.nix
{ osConfig, config, pkgs, ... }:
let
  user = "yusu";
  name = "Jonas Opitz";
...
#+begin_src nix :tangle home-manager/sisyphus.nix
{ osConfig, config, pkgs, ... }:
let
  user = "yusu";
  name = "Jonas Opitz";
...
#+begin_src nix :noweb yes :tangle home-manager/desktop.nix
programs.autorandr = {
  enable = true;
  profiles = {
    <<autorandr_profiles>>
...
#+begin_src nix :noweb yes :tangle home-manager/sisyphus.nix
programs.fish.interactiveShellInit = ''
  <<fish_init>>
  set EDITOR vim
'';
...
*** Vale
:PROPERTIES:
:header-args: :tangle home-manager/desktop.nix
:END:
...
[[id:080be9ce-24ea-4a89-a21f-aeb9a090faa4][Ledger]]: Plain Text Accounting
...
**** Dependencies
***** Doom-Emacs
Emacs dependencies that need to be globally installed
#+begin_src nix :tangle home-manager/desktop.nix
# emacs everywhere
pkgs.xorg.xwininfo
pkgs.xdotool
...
** Finalize
#+begin_src nix :noweb yes :tangle home-manager/core.nix
}
#+end_src
...
#+begin_src nix :noweb yes :tangle home-manager/hephaestus.nix
}
#+end_src

#+begin_src nix :noweb yes :tangle home-manager/orpheus.nix
}
#+end_src
